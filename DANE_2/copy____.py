...
class Ridge_regression:
	...
	def prox_Batch_SGD(self, v, mu_internal, mode , max_iter , w_loc , eval_mode , rate_param ):
		...
		x_new = []
		y_new = []
		# for j in np.arange( math.floor( B * max_iter/n ) + 1 ):
		# 	np.random.shuffle( shuffle )
		...
		# x_new = np.reshape( np.asarray( x_new ) , [ n*(math.floor( B * max_iter/n ) + 1), dim ])
		# y_new = np.reshape( np.asarray( y_new ) , [n*(math.floor( B * max_iter/n ) + 1)] )
	...
	def simple_optimize(self, mode , max_iter , w_loc , first_rate_param  , sgd_rate ,  dane_iter_number ):
		number_of_gradients = 0#self.n * 0.2
		# w_opt = self.prox_exact_inverse(v, mu_internal)
		...
		if mode == 'SGD':
			...
			eval_mode = 0
			w_opt , number_of_iters ,sgd_gamma = self.plain_SGD( mode , max_iter , w_loc , eval_mode, first_rate_param , sgd_rate, dane_iter_number )
			number_of_gradients = number_of_iters

		return w_opt , number_of_gradients , sgd_gamma
	...
	def plain_SGD(self, mode , max_iter , w_loc , eval_mode , first_rate_param , sgd_rate , dane_iter_number ):
		'''
		instead of exact computation use a few (maybe only one) run of SGD!
		mu_internal == 0
		...
		for i in xrange( max_iter ):
			...
			if first_rate_param == 'inverse_t':
				sgd_gamma = sgd_gamma_0 / ( 1 + 0.01 * ( i+1 ) )
			elif first_rate_param == 'inverse_t_sqrt':
				sgd_gamma = sgd_gamma_0 / ( 1 + 0.01 * math.sqrt( i+1 ) )
			# print 'this is SGD learning rate: ' , sgd_gamma
