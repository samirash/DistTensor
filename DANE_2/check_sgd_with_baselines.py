...
from distMiniBatchSGD import run_dist_minibatchSGD_ridgeregression_exp_2 , dist_minibatchSGD_procedure


def run_DANE_ridgeregression_exp_2(N, m, max_iter, flag, data, w_opt , mode , max_inner_iter , sampling_flag_rate , rate_param):
...
def ridgeregression_exp_2_inner_iter( mode , max_inner_iter ):
	...
	# JUST HISTORY OF WHAT I EXPERIMENTED WITH:
	...
	#  setting the number of inner iteration for SGD, we'll use the equivalet for mini-batch SGD or other methods
	T_base = int( math.ceil(experiment_data_size[0]/ 1. ) )
	T1 = T_base/5.
	T2 = T_base/2.
	T3 = T_base
	T4 = T_base * 1.5
	...
	experiment_modes = [     [ 'SGD' , T2 ]  , [ 'SGD' , T5 ]   ]#[    [ 'SGD' , T1 ]  , [ 'SGD' , T2 ]  , [ 'SGD' , T5 ]  , [ 'SGD' , T7 ]  ]# , [ 'SGD' , T3 ]    , [ 'SGD' , T4 ]  ,  [ 'SGD' , T5 ] ]
	...
	for k in range(len(experiment_modes)): # m is the number of machines,  4, 16
		for j in range(len(rate_params)):  # N is the number of points
			...
			print 'New experiment round:','i:', i ,'j:', j
			...
			evals_0, runtimes_0, w_ans_0 , w_opt_0, data_0 , number_of_gradients_0 , number_of_gradients_0_2 = run_DANE_ridgeregression_exp_2( N , 1 , optimal_iter , 0 , 0 , 0 , 'linearEq_exact'  , max_inner_iter , [0], rate_param )# drawing fresh data --> flag=0 , data is not given -> data = 0 , we don't have w_opt to give -> w_opt = 0
			...
			# evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_distSGD_ridgeregression_exp_2( N/m , 1 , max_iter , 1 , data_0 , w_opt_0 , mode , max_inner_iter/m , sampling_flag_rate ,  rate_param ) # flag=1, use the same data, w

			rate_params_temp = [[ 'inverse_t' , 0.5  , 5 ]  ]#, [ 'fix' , 0.001 ] , [ 'fix' , 0.002 ] ]
			...
			evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_dist_minibatchSGD_ridgeregression_exp_2( N , m , max_iter , 1 , data_0 , w_opt_0 , mode , max_inner_iter/(m*rate_params_temp[0][2]) , sampling_flag_rate ,  rate_params_temp[0] ) # flag=1, use the same data, w
			...
			max_inner_iter_ = max_inner_iter/m
			evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_DANE_ridgeregression_exp_2( N , m , max_iter , 1 , data_0 , w_opt_0 , mode , max_inner_iter_ , sampling_flag_rate ,  rate_param ) # flag=1, use the same data, w
			...
			evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_distSGD_ridgeregression_exp_2( N , 1 , max_iter*m , 1 , data_0 , w_opt_0 , mode , max_inner_iter/m , sampling_flag_rate ,  rate_param ) # flag=1, use the same data, w
	...
	def draw_plots(all_evals , all_optimals , all_suboptimalities , all_runtimes, all_gradient_counts , all_gradient_counts_2 , a, b , max_iter , versus_time_flag , m):
		...
		if versus_time_flag == 1:
			...
			print  'Is PLOTTING versus number of gradients now! .... ..... ......'
			...
			for i in range(a):
				...
				plt.plot(grad_counts[ 0: max_iter+1 , i*4 +1] , all_suboptimalities[ 0: max_iter+1 , i*4 +1], plot_colors[ 1 ] ,marker = 'v' ,  label='all_machines_all_data_dane')
...
# this is extra now, I'm setting them again inside the function, but keep it just to know the availabilities:
