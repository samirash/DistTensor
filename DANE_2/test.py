...
import math
from distSGD import run_distSGD_ridgeregression_exp_2, distSGD_procedure


def run_DANE_ridgeregression_exp_2(N, m, max_iter, flag, data, w_opt, mode, max_inner_iter, sampling_flag_rate,
                                          rate_param):
    ...
    evals, runtimes, w_ans, number_of_gradients, number_of_gradients_2 = DANE_procedure(machines, w_opt,
                                                                                        'ridge_regression',
                                                                                        objective_param, max_iter,
                                                                                        DANE_params[0], DANE_params[1],
                                                                                        mode, max_inner_iter,
...
def ridgeregression_exp_2_inner_iter(mode, max_inner_iter):
    max_iter = 20
    optimal_iter = 20
    ...
    T1 = int(math.ceil(experiment_data_size[0] / 5.))
    T2 = int(math.ceil(experiment_data_size[0] / 2.))
    T3 = int(math.ceil(experiment_data_size[0] / 1.))
    T4 = int(math.ceil(1.5 * experiment_data_size[0]))
    T5 = int(math.ceil(2.0 * experiment_data_size[0]))
    T6 = int(math.ceil(4.0 * experiment_data_size[0]))
    T7 = int(math.ceil(6.0 * experiment_data_size[0]))

    experiment_modes = [['SGD', T2], ['SGD',
                                      T5]]  # [    [ 'SGD' , T1 ]  , [ 'SGD' , T2 ]  , [ 'SGD' , T5 ]  , [ 'SGD' , T7 ]  ]# , [ 'SGD' , T3 ]    , [ 'SGD' , T4 ]  ,  [ 'SGD' , T5 ] ]
    ...
    experiment_size = len(experiment_modes) * len(rate_params) * 4
    print experiment_size

    m = 4
    all_suboptimalities = np.zeros((m * max_iter + 1, experiment_size))
    all_evals = np.zeros((m * max_iter + 1, experiment_size))
    all_runtimes = np.zeros((m * max_iter + 1, experiment_size))
    all_optimals = np.zeros((1, experiment_size))

    all_gradient_counts = np.zeros((m * max_iter + 1, experiment_size))
    all_gradient_counts_2 = np.zeros((m * max_iter + 1, experiment_size))
    ...
    for k in range(len(experiment_modes)):  # m is the number of machines,  4, 16
        for j in range(len(rate_params)):  # N is the number of points
            ...
            evals_0, runtimes_0, w_ans_0, w_opt_0, data_0, number_of_gradients_0, number_of_gradients_0_2 = run_DANE_ridgeregression_exp_2(
                N, 1, optimal_iter, 0, 0, 0, 'linearEq_exact', max_inner_iter, [0],
                rate_param)  # drawing fresh data --> flag=0 , data is not given -> data = 0 , we don't have w_opt to give -> w_opt = 0
            ...
            # evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_DANE_ridgeregression_exp_2( N/m , 1 , max_iter , 1 , data_0 , w_opt_0 , mode , max_inner_iter_ , sampling_flag_rate ,  rate_param ) # flag=1, use the same data, w
            evals, runtimes, w_ans, w_opt, data, number_of_gradients, number_of_gradients_2 = run_distSGD_ridgeregression_exp_2(
                N / m, 1, max_iter, 1, data_0, w_opt_0, mode, max_inner_iter / m, sampling_flag_rate,
                rate_param)  # flag=1, use the same data, w
            ...
            max_inner_iter_ = max_inner_iter / m
            evals, runtimes, w_ans, w_opt, data, number_of_gradients, number_of_gradients_2 = run_DANE_ridgeregression_exp_2(
                N, m, max_iter, 1, data_0, w_opt_0, mode, max_inner_iter_, sampling_flag_rate,
                rate_param)  # flag=1, use the same data, w
            ...
            i = i + 1
            print '	HEEEEEEEEEEREEEEEEEEEE: .............. (4)'
            # running on one machine with all data:
            # max_inner_iter_ = max_inner_iter/m
            # evals, runtimes, w_ans , w_opt, data , number_of_gradients , number_of_gradients_2 = run_DANE_ridgeregression_exp_2( N , 1 , max_iter , 1 , data_0 , w_opt_0 , mode , max_inner_iter , sampling_flag_rate ,  rate_param ) # flag=1, use the same data, w
            evals, runtimes, w_ans, w_opt, data, number_of_gradients, number_of_gradients_2 = run_distSGD_ridgeregression_exp_2(
                N, 1, max_iter * m, 1, data_0, w_opt_0, mode, max_inner_iter / m, sampling_flag_rate,
                rate_param)  # flag=1, use the same data, w
            ...
            print 'OK! norm_ of _w_opt (original w we made data with) *** , ', np.dot(w_opt,
                                                                                      w_opt)  # the algorithm finds w with smaller w (regularization term)
    ...
    all_suboptimalities = all_evals - np.repeat(all_optimals, m * max_iter + 1, axis=0)
    ...
    def draw_plots(all_evals, all_optimals, all_suboptimalities, all_runtimes, all_gradient_counts,
                   all_gradient_counts_2, a, b, max_iter, versus_time_flag, m):

        all_suboptimalities = np.log10(all_suboptimalities)
        all_suboptimalities = all_suboptimalities.clip(-3)
        m = 4
        t = np.arange(m * max_iter + 1)
        ...
        if versus_time_flag == 1:
            ...
            for i in range(a):
                ...
                crop_point = max(grad_counts[-1, i * 4], grad_counts[-1, i * 4 + 1], grad_counts[-1, i * 4 + 3] / m);

                ax = plt.subplot(111)
                ax.set_xlim([0.0, crop_point])

                plt.plot(grad_counts[0: max_iter + 1, i * 4], all_suboptimalities[0: max_iter + 1, i * 4],
                         plot_colors[0], marker='o', label='single_machine_total/m_data')

                plt.plot(grad_counts[0: max_iter + 1, i * 4 + 1], all_suboptimalities[0: max_iter + 1, i * 4 + 1],
                         plot_colors[1], marker='v', label='all_machines_all_data_dane')

                plt.plot(grad_counts[:, i * 4 + 2], all_suboptimalities[:, i * 4 + 2], plot_colors[2], marker='+',
                         label='single_machine_all_data')

                plt.plot(grad_counts[:, i * 4 + 3] / m, all_suboptimalities[:, i * 4 + 3], plot_colors[3], marker='*',
                         label='single_machine_all_data_devided_by_m')
    ...
    draw_plots(all_evals, all_optimals, all_suboptimalities, all_runtimes, all_gradient_counts, all_gradient_counts_2,
               a, b, max_iter, 1, experiment_machines_number[0])
...
ridgeregression_exp_2_inner_iter(mode, max_inner_iter)
